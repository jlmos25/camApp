...
import com.msqr.server_list.ui.ServerListUiState
...
class ShowServersViewModel(private val serverRepository: ServerRepository) :ViewModel() {
    ...
    private val _serverListUIStateFlow = MutableStateFlow(ServerListUiState())
    val serverUiState = _serverListUIStateFlow.asStateFlow()
        ...
        viewModelScope.launch {
            serverRepository.findAllServers().collectLatest { serverList ->
                ...
                if (serverList.isEmpty()){
                    ...
                    _serverListUIStateFlow.update { it.copy(servesList = serverList) }
        ...
        viewModelScope.launch {
            serverRepository.addServer(server).collectLatest{response ->
                when(response){
                    is Response.Success -> {
                        if(response.data){
                            val tmpList = _serverListUIStateFlow.value.servesList.toMutableList()
                            tmpList.add(server)
                            _serverListUIStateFlow.update { it.copy(servesList = tmpList) }
                            _serverViewEvent.value = ServersViewEvent.SHOW_SERVER_LIST
                        }else {
